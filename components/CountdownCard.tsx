  ...
  return (
    <article style={{
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", gap: 8 }}>
        ...
        {pinned && <span style={{ fontSize: 10, color: "#7aa2f7", letterSpacing: 1, flexShrink: 0 }}>PINNED</span>}
      </div>
      <div style={{ fontVariantNumeric: "tabular-nums", fontSize: 20, fontWeight: 600, letterSpacing: 0.5, whiteSpace: "nowrap" }}>
        {d}d {pad(h)}h {pad(m)}m {pad(s)}s
      </div>
      <div style={{ fontSize: 12, color: "#7a8595", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{when}</div>
    </article>
  );
}
